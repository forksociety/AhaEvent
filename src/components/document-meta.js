...
class DocumentMeta extends Component {
    render() {
        let title = (validProp(this.props.title) ? this.props.title.trim() + " | " : "") + config.get('siteName');
        let description = validProp(this.props.description)
            ? this.props.description.trim()
            : config.get("siteDescription");
        let keywords = validProp(this.props.keywords) ? this.props.keywords.trim() : config.get("siteKeywords");

        let ogTitle = title;
        let ogDescription = description;
        let ogUrl = validProp(this.props.ogUrl) ? this.props.ogUrl.trim() : config.get("siteUrl");
